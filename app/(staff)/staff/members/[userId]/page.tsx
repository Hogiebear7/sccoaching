...
export default async function StaffMemberDetailPage({
  ...
  // Free and App Subscription tier members' owned data stays fully behind
  // this wall until they hold a Membership-tier subscription — see
  // lib/member-tier-wall.ts. Everything below that reads workout/recovery/
  // nutrition/messages/emergency-contact data is skipped entirely while
  // walled, not just hidden in the JSX, so none of it ever reaches the
  // rendered HTML.
  const subscription = findSubscriptionByUserId(user.id);
  const walled = !staffCanViewMemberData(user.id);

  const profile = findProfileByUserId(user.id);
  const cycleSettings = !walled && profile?.cycleTrackingEligible
    ? findCycleSettingsByUserId(user.id)
    : undefined;
  const cyclePrivacy = !walled && profile?.cycleTrackingEligible
    ? findCyclePrivacyByUserId(user.id)
    : undefined;
  const pregnancyStatus = !walled && profile?.cycleTrackingEligible
  ...
  const phaseEstimate = cycleSettings
    ...
    : null;
  const programme = walled ? null : findProgrammeByUserId(user.id);
  const sessions = walled ? [] : findWorkoutSessionsByUserId(user.id);
  const personalBests = computePersonalBests(sessions);
  const coachNote = walled ? undefined : findCoachNoteByUserId(user.id);
  const recoveryLogs = walled ? [] : findRecoveryLogsByUserId(user.id).slice(0, 7);
  ...
  // Skipped while walled: a walled member can't message staff at all (see
  // the tier gate in app/api/messages/send/route.ts), so there's nothing to
  // mark read, and no thread should be touched or fetched.
  const messages = walled ? [] : (markMemberMessagesReadByStaff(user.id), findMessagesByMemberId(user.id));
  ...
  const drinkSettings = walled ? null : profile?.drinkSettings ?? null;
  ...
  return (
    <section className="space-y-6">
      ...
      {!walled ? <CoachSummaryPanel memberId={user.id} /> : null}

      {!walled ? <AiUsagePanel memberId={user.id} /> : null}
      ...
      {walled ? (
        <div className="panel p-6">
          <h3 className="text-lg font-semibold">Free / App Subscription tier</h3>
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            This member&apos;s profile, workouts, nutrition, recovery, and messages stay private until
            they hold a Membership-tier subscription. Grant Membership above to unlock their data.
          </p>
        </div>
      ) : !profile ? (
        <div className="panel p-6">
      ...
      {!walled && profile ? <DietaryRequirementsSummary profile={profile} /> : null}

      {/* Cycle tracking */}
      {!walled && profile?.cycleTrackingEligible ? (
        <div className="panel p-6">
      ...
      {!walled && profile?.cycleTrackingEligible && pregnancyStatus?.isPregnant ? (
        <div className="panel p-6">
