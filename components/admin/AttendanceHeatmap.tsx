...
function intensity(count: number, max: number): string {
  ...
  const t = count / max;
  if (t < 0.2) return "bg-teal-900/50";
  ...
  return (
    <div className="overflow-x-auto">
      <div className="min-w-[480px]">
        {/* Day headers */}
        <div className="flex pl-10 gap-px mb-1.5">
          {DAYS.map((d) => (
            <div key={d} className="flex-1 text-center text-[10px] font-medium tracking-wide text-zinc-500">{d}</div>
          ))}
        </div>
        {/* Rows: hour × day */}
        {HOURS.map((label, hi) => {
          ...
          return (
            <div key={hour} className="flex items-center gap-px mb-px">
              <div className="w-10 text-[9px] text-zinc-600 text-right pr-1.5 flex-shrink-0 tabular-nums">{label}</div>
              {DAYS.map((_, di) => {
                ...
                return (
                  <div
                    ...
                    className={`flex-1 h-5 rounded-[3px] ${intensity(count, max)} transition-colors`}
                  />
                );
              })}
            </div>
        ...
        <div className="flex items-center gap-1.5 mt-3 pl-10">
          ...
          {["bg-white/[0.03]", "bg-teal-900/50", "bg-teal-700/70", "bg-teal-500"].map((c, i) => (
